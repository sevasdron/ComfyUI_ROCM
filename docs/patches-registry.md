...
| P3 | Блокировка Linux в `_platform_spec()` + поиск локального `llama-cli` | ComfyUI-LLM-text-processor | node | 3 | не исправлено (65983de) | **сделано в `main`** (15.09): `modpacks/main/patches/llm-text-processor-linux.patch` в `nodes.lock`; `llama-cli` b10472 — `~/ComfyUI/main/bin/llama.cpp/current` (путь от каталога ноды: `/data/bin/…`); ставит `tools/llama-fetch`, с 17.09 сборка **Vulkan** (RADV в образе через `dnf` modpack.yaml), замер в rocm-accelerators.md |
