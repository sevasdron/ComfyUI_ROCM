...
| P6 | Sage Attention на AMD не работает, у ноды нет выключателя | KJNodes `MiniMaxH3MemoryEfficientSageAttentionPatch` | workflow | 7 | — | **закрыто 16.09**: замена — нода ядра `Model Attention Backend` с backend `comfy kitchen attention` (INT8 через HIP WMMA, cos 0.99988). Записано в [node-policy.yaml](../modpacks/main/node-policy.yaml), замеры — [rocm-accelerators.md](rocm-accelerators.md) |
