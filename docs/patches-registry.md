...
| P6 | Sage Attention на AMD не работает, у ноды нет выключателя | KJNodes `MiniMaxH3MemoryEfficientSageAttentionPatch` | workflow | 7 | — | падение подтверждено прогоном 16.09 (`RuntimeError: sageattention is not new enough version…`); **кандидат на замену** (прогоном не подтверждён): нода ядра `Model Attention Backend` с backend `comfy kitchen attention` — INT8 через HIP WMMA, на синтетике cos 0.99988. До первого прогона Bypass остаётся рабочим вариантом. [node-policy.yaml](../modpacks/main/node-policy.yaml), замеры — [rocm-accelerators.md](rocm-accelerators.md) |
