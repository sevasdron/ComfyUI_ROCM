...
| P3 | Блокировка Linux в `_platform_spec()` + поиск локального `llama-cli` | ComfyUI-LLM-text-processor → с 17.09 форк `Kvento/ComfyUI-LLM-text-processor_mtp_ND` (MTP; тот же тип ноды, заменяет пакет целиком) | node | 3 | не исправлено (65983de; форк 9976aed — то же) | **сделано в `main`** (15.09, перенесено на форк 17.09): `modpacks/main/patches/llm-text-processor-mtp-linux.patch` в `nodes.lock` (код бинарника в форке живёт в `nodes.py`); `llama-cli` b10472 — `~/ComfyUI/main/bin/llama.cpp/current` (путь от каталога ноды: `/data/bin/…`); ставит `tools/llama-fetch`, с 17.09 сборка **Vulkan** (RADV в образе через `dnf` modpack.yaml), замер в rocm-accelerators.md |
...
| P6 | Sage Attention на AMD не работает, у ноды нет выключателя | KJNodes `MiniMaxH3MemoryEfficientSageAttentionPatch` | workflow | 7 | — | **закрыто 16.09 заменой**: нода падает на ROCm (`RuntimeError: sageattention is not new enough version…`), вместо неё — нода ядра `Model Attention Backend` с backend `comfy kitchen attention` (INT8 через HIP WMMA). Подтверждено прогоном: шаг сэмплинга MiniMax H3 79.5 с против 417 с (5.25x). [node-policy.yaml](../modpacks/main/node-policy.yaml), замеры — [rocm-accelerators.md](rocm-accelerators.md) |
...
| P15 | Lora Manager: сборщик метаданных берёт `samples.shape[3]` у латента, проверив лишь `len(shape) >= 3`; аудио-латент трёхмерный → `IndexError: tuple index out of range` в логе на каждом аудио-прогоне (ошибка ловится, но пишется как ERROR) | ComfyUI-Lora-Manager `py/metadata_collector/node_extractors.py`, `extract_latent_dimensions` | node | — | не исправлено (bf497d5) | **сделано 17.09**: `modpacks/main/patches/lora-manager-audio-latent.patch` — проверка `>= 4`; подтверждено прогоном ACE Step после перезапуска (74 с, в логе ни одной ошибки); кандидат в PR upstream (1 строка) |
