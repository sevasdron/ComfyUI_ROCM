...
Жёсткий способ есть, и он в самом пакете: нода `FL_YuE2_Plan` принимает готовую партитуру во вход `score_abc`
и тогда не сочиняет свою. Цепочка «Plan (стиль + текст) → `score_abc` → `RegexReplace` (`^Q:1/4=\d+` →
`Q:1/4=<нужный BPM>`) → второй Plan с тем же стилем и текстом → Render» делает темп настройкой: сочинение
идёт один раз, второй Plan только токенизирует партитуру. Не построено, пока идея; проверять придётся на слух —
партитура, сочинённая под 90, растянутая до 140 через строку темпа, поёт быстрее, но фразировка остаётся «медленной».
