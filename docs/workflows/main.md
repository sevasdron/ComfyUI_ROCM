...
Ползунки на холсте (все четыре — mxSlider, значения регулируются перетаскиванием):

| Ползунок | Где | Значение | Диапазон / шаг |
|---|---|---|---|
| `Duration | Seconds` | левая колонка, y 2569 | 15 с | 4–120, шаг 0.5 |
| `Segment | Seconds` | левая колонка, y 2636 | 5 с | 1–60, шаг 0.5 |
| `Context | Tokens` | правая колонка, y 2569 | 16384 | 2048–32768, **шаг 512** |
| `Max Tokens` | правая колонка, y 2603 | 6144 | 1024–16384, **шаг 512** |

Грабли: сначала оба новых ползунка легли поверх `Main | Steps` в той же колонке и выглядели как «их нет».
Перед тем как добавлять ноду на холст, смотри, что уже стоит в этих координатах (в редакторе:
`app.graph._nodes` с фильтром по `pos`).

Поля `ctx_size` и `max_tokens` на самой ноде `#9430` остаются видимыми, но неактивными — значение приходит
по связи с ползунка, это штатное поведение ComfyUI. Убрать их из ноды нельзя: фронтенд восстанавливает
виджеты подграфа по схеме, а попытка сделать их обычными сокетами сдвигает значения соседних виджетов.
