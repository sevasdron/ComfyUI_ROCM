...
Вторая копия `ND_YuE2_T2M_v1.2_MTP_RH` — авторская версия с MTP: генератор текста с головой-черновиком
(`mtp` на ноде `#27`), модель Gemma4-26B-A4B как у автора (лежит в корне `LLM/`, голова в `LLM/MTP/` — путь в файле
поправлен на `MTP/mtp-gemma-4-26B-A4B-it.gguf`, у автора он без подпапки). Пакет ноды с 17.09 — форк с MTP,
тот же тип `LLMTextProcessor`, в сохранённых воркфлоу значения не сдвинулись (проверено в браузере: VVJ, YuE2),
новый виджет `mtp` встаёт с `none`. llama.cpp на Vulkan (см. rocm-accelerators.md).
