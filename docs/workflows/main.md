...
### Новый виджет в ноде сдвигает значения во всех сохранённых воркфлоу

Фронтенд (1.51) при загрузке раскладывает `widgets_values` **по позиции**, `widgets_values_named` он только пишет,
при чтении не использует. Когда пакет ноды добавляет виджет в середину (форк LLM Text Processor вставил `mtp`
третьим), каждый сохранённый воркфлоу с этой нодой едет: в `mtp` попадает системный промпт, в `max_tokens` —
температура, в `ctx_size` — режим памяти и так далее, тихо, без ошибки при открытии. Ловится только сравнением
значений после `loadGraphData` (проверять на **свежей** вкладке: старая держит определения нод с прошлого
запуска сервера и покажет «всё в порядке»). Лечится правкой файлов: вставить умолчание на нужную позицию
`widgets_values` и в `widgets_values_named`; если нода сохранена новым фронтендом, у неё есть ещё список `inputs`
со всеми виджетами — туда тоже вставить запись `{name: mtp, widget: {name: mtp}}` после `mmproj`, иначе порядок
строится по этому списку и сдвиг остаётся. Проверять такое при каждой замене пакета ноды на форк. И ещё одни грабли
при проверке: `fetch('/api/userdata/…')` в браузере отдаёт файл из HTTP-кэша — после правки на диске читать с
`{cache: 'no-store'}`, иначе проверяешь старую версию (так 17.09 VVJ полчаса «не чинился»).
...
тот же тип `LLMTextProcessor`, но с новым виджетом `mtp` третьим по счёту — и это **сдвинуло значения во всех
сохранённых воркфлоу** со старой нодой (см. «грабли» ниже); 17.09 все файлы в `user/default/workflows` поправлены:
в `widgets_values` вставлен `none` на позицию `mtp`. llama.cpp на Vulkan (см. rocm-accelerators.md).
...
Вечер 17.09, серия прогонов уже с моделями на диске (`YuE2_00002`–`00010.flac`): 282–382 с на трек при
авторских настройках, по одному разу подряд без перезагрузок. Оценка пользователя по звуку — отлично: интонации, музыка, сведение; воркфлоу принят как рабочий.
Открытый вопрос — время (5–6 минут на трек): смотреть скрытые в подграфе `#12` параметры (`acoustic_steps` по
`quality` 16/40/64, `max_duration`, `planning`, `guidance`) и замерить, где уходит время — AR-токены музыки или
акустический солвер. В одном прогоне ядро написало `YuE2 reached max_duration; increase it if the song
ends early` — это не ошибка: трек упёрся в верхнюю границу `max_duration` на `#12`, если конец обрезан — поднять.
Текст в этих прогонах писал генератор на `#27` (форк с MTP, llama.cpp на Vulkan) — отдельного замера LLM нет,
в лог ComfyUI llama-cli не пишет.
...
### Темп: почему BPM в тегах «не слушается»

В отличие от ACE Step, у YuE2 нет полей темпа, тональности и длительности. Модель работает в три стадии: сначала
пишет ABC-партитуру (у неё есть строка темпа `Q:1/4=<BPM>`, размер `M:`, тональность `K:`), потом по ней
генерирует токены музыки, потом акустику. Темп задаёт партитура; слова «slow ballad» и «150 BPM» в стиле — лишь
подсказка для стадии сочинения, модель может её не выполнить. Длительность — от объёма текста (`max_duration`
только обрезает). Упоминание BPM в стиле оставить (это единственный мягкий рычаг), но рассчитывать на него нельзя.

Жёсткий способ есть, и он штатный — редактор партитуры. В пакете FL это нода `FL YuE2 · Piano Roll`
...
самодельная цепочка с `RegexReplace`: редактор с темпом был в пакете с самого начала (18 типов нод), не посмотрел.

### Официальные ноды YuE2 в ядре (с v0.36.0)

С ComfyUI v0.36.0 в ядре есть своя реализация: `YuE2GenerateABC` (стиль + текст → ABC-партитура),
`YuE2GenerateMusic` (стиль, текст, `abc`, `max_duration` → CONDITIONING + секунды), `EmptyYuE2LatentAudio`,
обычный `KSampler` (в шаблоне 32 шага, `dpm_2`, `sgm_uniform`, cfg 1), `VAEDecodeAudio`; для каверов —
`SheetSage2AudioToABC` (аудио → партитура через `AudioEncoderLoader`). Поля BPM там тоже нет: темп словами в тегах
...
### Русский язык и обучение: что выпущено, а что нет (18.09)

Наблюдение пользователя: хотя на демо-сайте русский пример один (Folktronica), модель на русском тексте хорошо
держит и остальные представленные там стили — язык и жанр у неё независимы.

Про «есть декодер, нет энкодера». Проверено по файлам: в `YuE2-Vae/model.safetensors` лежат **обе** половины
акустического VAE (217 тензоров decoder, 218 encoder); инференс-загрузчик FL берёт только декодер, энкодер
использует обучающая часть пакета. Не выпущен другой компонент — **семантический аудио-токенайзер**: модель,
которая переводит запись в дискретные «семантические токены», стоящие внутри YuE2 между партитурой и акустикой.
В коде авторов он назван отдельным невыпущенным компонентом; MERT-v2 (он выпущен) даёт непрерывные признаки и
заменой не является (это прямо сказано в `skills/yue2-music/references/models-and-setup.md`). Следствие
(разбор в `docs/TRAINING_RESEARCH.md` пакета FL): LoRA на **сочинение партитур** обучается уже сейчас (аудио-токены
не нужны), акустическая LoRA по парам «условие → латенты VAE» возможна как эксперимент, а обучение семантической
стадии на своих записях упирается в отсутствующий токенайзер — его пришлось бы обучать самому (кодбук, совместимый
с весами YuE2, без данных и рецепта авторов восстановить нельзя; реалистичен лишь обход: учить на токенах, которые
сгенерировала сама модель). Технический отчёт авторов заявлен как forthcoming.
