...
1. `promote` кандидата v0.36.0 (нужен для `SheetSage2AudioToABC` и нативных нод YuE2), проверка ACE Step и Krea на свежей вкладке.
2. Нода пака «разбор ответа усилителя» (секции `### STYLE / LYRICS / TAGS / META` → отдельные выходы) и подключение усилителя
   ...
   *18.09: ноды `RH Music Target / Prompt Parse / Pick` и воркфлоу `Audio/RH_Music_Style_Generator` собраны; текстовая
   часть проверена прогоном на кандидате (8101) для всех трёх моделей, генерация музыки — нет. Шаг 1 (promote) ещё не сделан.*
